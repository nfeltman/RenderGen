...
j("monopatt",			"let val grnd{(x,y)} = grnd{(1,2)} in grnd {x+y}",SAME),
...
j("if2", 				"next{ if 2 > 3 then prev{hold grnd{4 + 6}} else 2 * 3}",SAME),
j("ifBothSides",		"next{ if 2 > 3 then prev{hold grnd{4 + 6}} else prev{hold grnd{2 * 3}}}",SAME),
j("ifPred",				"next{ if 2 > prev{hold grnd{2 + 4}} then prev{hold grnd{4 + 6}} else prev{hold grnd{2 * 3}}}",SAME),
j("if7",				"if 3 > 2 then next{prev{hold grnd{8 * 9}} + 6} else next{prev{hold grnd{8 + 9}} * prev{hold grnd{6 - 2}}}",SAME),
j("holdif",				"hold grnd{if 2 > 3 then 1 else 0}",SAME),
j("holdif2",			"next {prev {hold grnd{if 2 > 3 then 1 else 0}}}",SAME),
...
j("multiStageFunc", 	"(fn grnd{x} : ^int => next{prev{hold grnd{x * x}} + prev{hold grnd{x}}}) grnd{45}",SAME),
...
j("datastruct2", 		"let fun f (grnd{x}:^int) = next{prev{hold grnd{x*x}}+4} in ((f grnd{1},f grnd{2}),(f grnd{3},f grnd{4}))",SAME),
j("datastruct3", 		"let fun map (f : ^int -> $int) = " ^ 
						"fn M:((^int*^int)*(^int*^int)) => ((f (#1 (#1 M)), f (#2 (#1 M))), (f (#1 (#2 M)), f (#2 (#2 M)))) in " ^ 
						"map (fn grnd{x}:^int => next{prev{hold grnd{x*x}}+4}) ((grnd{1}, grnd{2}), (grnd{3}, grnd{4}))",SAME),
