...
struct
	...
	fun inferType e = 
			let 
				...
				fun maybeBox Single t = t
				  | maybeBox Multi t = Tarray t
				...
				fun chE expr g =
					case expr of
					  Echain (innerType, e1, e2) => (
							case (innerType, chE e1 g, chE e2 g) of 
							  (d0, (s1, d1 as GeoSamps _), (d2 as GeoSamps _, s2)) => (sameS d0 (sameS d1 d2 (BadType "chain")) (BadType "chain"); (s1,s2))
							| (Hits d0, (s1, Hits d1), (Hits d2, s2)) => (sameD d0 (sameD d1 d2 (BadType "chain")) (BadType "chain"); (s1,s2))
							| (Frags d0, (s1, Frags d1), (Frags d2, s2)) => (sameD d0 (sameD d1 d2 (BadType "chain")) (BadType "chain"); (s1,s2))
                    ...
                    | EboundG (s, ms) => (GeoSamps (maybeBox ms Tflat, s), GeoSamps (maybeBox ms (Tbounded Tflat), s))
                    | EunboundG (s, ms) => (GeoSamps (maybeBox ms (Tbounded Tflat),s), GeoSamps (maybeBox ms Tflat,s))
                    | EboundS (g, ms) => (GeoSamps (g, maybeBox ms Tflat), GeoSamps (g, maybeBox ms (Tbounded Tflat)))
                    | EunboundS (g, ms) => (GeoSamps (g, maybeBox ms (Tbounded Tflat)), GeoSamps (g, maybeBox ms Tflat))
