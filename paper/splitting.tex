...
% and second-stage computations in the original term. 

% Recall that full evaluation (both the first and second pass) of a term $\coltwo e A$ 
% only produces its ``final result'' at the second stage.
% In contrast, \bbonem\ terms may have final results at both stages. 
% For example, consider the term:
% \begin{lstlisting}
% 1`(`3`gr{`1`4+5`3`}`1`, next{`2`6+7`1`})`
% \end{lstlisting}


% Thus, the idea of splitting for \bbtwo\ terms
...
\TODO why the added complexity for \bbonem\ terms?
...
Since execution of the first pass both serves to generate input for
the second pass, we say $c$ is a {\em combined term} that produces a
\emph{immediate result} ($i$) and a boundary value ($b$) for the resumer ($l.r$).

The correctness of \bbonem\ term splitting with respect to $\stepsym \bbonem$ is given in \ref{lem:oneCorrect}.
...
\TODO talk about lifting step correctness

\TODO talk about ground case
