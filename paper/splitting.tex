...
Note that this result carries information at {\em both} stages: 
at stage~\bbone\ it carries the value 3, and at stage~\bbtwo\ it carries the code $\letin \yhat {\mathtt{7+5}} \yhat$. 
Therefore, to make the results of splitting match the the partial value above, we choose to split \bbone\ terms to create a resumer that reduces to the same value as the stage~\bbtwo\ carried by the partial value, and an $e1$ that not only performs the precomputation necessary for the resumer, but also generates values that match the stage~\bbone\ aspects of the partial value.
...
To concretize what it means by ``match", we introduce two {\em masking} functions, $\masko{\cdot}$ and $\maskt{\cdot}$, that project from the residual table and partial value only the parts that are relevant at a particular stage.  These functions are shown in \ref{fig:valMask}. They operate by replacing all stage \bbone\
