...
To see why, consider a function with a monostage body. 
For example, one which simply doubles its argument:
\begin{lstlisting}
2`prev{
  1`let fun double (n : int) = n + n
  in next{`2`hold {`1`double 5`2`}-7`1`}`2`
}`
\end{lstlisting}
Using the rule for splitting at \bbonem, this term splits into:
\begin{lstlisting}
1`let fun double (n : int) = (n+n, ())
in double 5`

2`l => 
let fun double (n : unit, l0 : unit) = ()
...
Optimizing such an inefficiency away after-the-fact is in general a global program analysis,
since functions like \texttt{double} could be passed around as values.  
