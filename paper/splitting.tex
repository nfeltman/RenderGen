...
Exactly what content is implied by this goal depends on the form of the input. 
There are four cases: stage \bbone\ partial values, stage \bbone\ terms, stage \bbtwo\ values,
...
\subsection {Stage \bbtwo\ Terms}

We cover stage \bbtwo\ term splitting first, because it has the most familiar signature.
In spirit, splitting takes a term $\coltwo e A$ and produces from it a first stage term
$p : \tau$ (called the {\em precomputation}) and a second stage term $r : \tau \to A$ (called the {\em resumer}).
Here, the responsibility of $p$ is to produce an intermediate data structure, 
and the responsibility of $r$ is to consume that data structure and produce the same final value as $e$.

Our actual presentation of stage splitting differs from this ideal in two ways.
Firstly, the outputs are untyped, for reasons discussed later.
Secondly, we do not actually represent the resumer as a function 
but instead as a term open on a variable, typically $l.r$.
This helps keep the splitting rules concise.
...
This is enough to define a correctness goal for splitting closed terms.

\paragraph {Correctness of stage \bbtwo\ splitting.}
If $e : A \splittwosym [p,l.r]$, then $e \equiv (\letin l p r)$


\subsection {Stage \bbtwo\ Values}

We next consider splitting for the special case of stage \bbtwo\ values.
Observing that no stage \bbtwo\ expression both is a value and contains stage \bbone\ subexpressions, 
we say that stage \bbtwo\ values must contain no stage \bbone\ content.
That is, stage \bbtwo\ value splitting is trivial by construction.

When applied to partial values, stage \bbtwo\ term splitting always produces a trivial
...
\subsection{Stage \bbone\ Partial Values}

Partial values, along with future environments, are produced by partially evaluating stage \bbone\ terms. 
They, unlike stage \bbtwo\ values, {\em can} have content at both stages.
As discussed above, the {\tt quickselect} function itself is an example of a multi-stage values.

Although partial values have no more stage
\bbone\ computation remaining, they may contain $\next$ blocks
(the contents of which must be variables bound by the future environment).
The goal of splitting for stage \bbone\ partial values is therefore 
to separate the contents of $\next$ blocks from the rest of the value, while maintaining some structure.
