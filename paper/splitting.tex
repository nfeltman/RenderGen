...
splitting yields a combined result formed by tupling $e$ with a \texttt{()} precomputation, and the trivial resumer \texttt{()}. (Stage~\bbone\ terminals, by definition, contain no stage~\bbtwo\ content.)  For example, \texttt{3} splits into the combined result \texttt{(3,())} and resumer \texttt{\_=>()}.
...
% We call attention to the rule for {\tt case}s since it, and the rule for $\pause$, where splitting adds non-trivial logic to the precomputation.
