...
\ur{Why is there a free variable ``Gamma'' in the figure for splitting
  stage 2?}
...
the variable $l$. The correctness of splitting thus requires that evaluating
$\letin l p r$ agrees with the value to which $e$ itself evaluates. 

\ur{I would not say that ``The correctness of splitting requires'',
  because we don't know what splitting is.  Instead, it is better to
  state this actively, e.g., say that we wish to split the code such
  that the condition e = let l = p in r holds.  correctness will have
