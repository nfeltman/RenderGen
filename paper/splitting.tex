...
\label{sec:splitting}
\ur{First para should probably be somewhere else concerning problem
  definition and relationship to PE.  Based on this definition, it
...
\ur{Reduce commentary.  Two sentences connected with an ``and.'' Split
  sentences.}
...
\ur{Next sentence is unnecessary commentary. It also sounds of
  questinoable accuracy, not clear to me that there are many ways to
  do this.}  There are many reasonable ways to achieve this goal, and
...
\ur{Drop, ``the most we can say.''  Be direct and explain.}
...
\footnote{We also used the term {\em residual} for the result of
  ...
  these concepts by using the terms {\em splitting-residual} and {\em
    evaluation-residual}.}  \ur{The reuse of residual is bothersome,
  we should use different terminology, 1-value and 2-value perhaps.}

\ur{We need a ``next'' I believe.  All our examples should start at
  stage-1 for consistency.}
...
\ur{I have no idea what the next paragraph is trying to say.}
...
\ur{``for both'' below is ambiguous.}
...
\ur{Drop commentary about taste and such.  Explain that we could have
  used lambda calculus as target but this is more direct.}  

\ur{Drop ``specifically''.}
...
open variables of $e$.
\ur{Case (pl./si.) mismatch in ``we represent ...'' sentence above.}
