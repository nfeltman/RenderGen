...
yields a pair of monostaged terms {$p$|$l.r$}. ($p$ and $l.r$
...
{$c$|$l.r$}. Evaluating $c$ in the first pass produces the pair
...
% splitting rules are local term driven

% generate a lot of cruft

% for example, case pushes the state of the case through to 2 even if branches have no stage 1 content

% recursive case would be recursive calls

% Rather than try to optimize globally So we use the type system to prove statically there no stage 1 stuff

% skip splitting.

% example is partition in 6.3
