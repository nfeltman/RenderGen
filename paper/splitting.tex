...
\ur{Technically, target language is not lambda but lambda plus pairs.
  Please use some other symbol than lambda, lambda-p lambda+ or
  something like that, or simply:} \textsf{\bf \textsc{Mono}}.
...
defining a masking relation that helps us to isolate stage-1 and
stage-2 components of a partial value and then move on to the
specification of the splitting algorithms.

\subsection{Partial Value Masking}

{\em Partial-value masking} or simply {\em masking} are a functions
for each stage that project from the domain of a partial value and
residual table to the a term in the target language \langmono.

\ur{I am attempting to define this even though it is not clear from
  the text what it is.  The definition should specify precisely the
  domain and the range of the functions.  Does the domain include the
  table?  The figure does not.  }


\ref{fig:valMask} defines the functions, $\masko{\cdot}$ and
$\maskt{\cdot}$. Since \texttt{next}'s only contain references to the
residual table (computations that occur in stage~\bbtwo),
$\masko{\xi;v}$ is to replaces all~$\next$s in~$v$ with~\texttt{()}.
The result is a \langmono\ value with the same structure as~$v$, but
containing only its stage~\bbone\ information. Masking is more complex
when~$v$ is a function since the function's body itself may contain
stage~\bbtwo\ subterms.  As a result, masking of functions depends on
the general behavior of stage~\bbone\ splitting, described shortly.

While $\masko{\cdot}$ produces a value in \langmono, $\maskt{\xi;v}$
...
\ur{There seems to a typo in ``unroll'' case in the figure.}
