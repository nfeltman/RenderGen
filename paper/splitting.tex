...
%For example, the partial value below contains both a stage~\bbone\ value 3, and a stage~\bbtwo\
...
%\end{lstlisting}
%which evaluates via $\redonesym$ to a 
%
%residual table and multi-stage tuple
...
returning the stage~\bbtwo\ component. After discarding stage~\bbone\ content from $v$, stage~\bbtwo\ masking reifies $\xi$ around the result,
...
The new combined term of $e$ then works by calling $c_1,\ldots,c_n$,
destucting them into variables $y_1,\ldots,y_n$ for immediate results 
and $z_1,\ldots,z_n$ for precomputations.
The immediate result of $e$ is then formed by replacing its subterms with $y_1,\ldots,y_n$, 
and the precomputations is formed from the tuple of $z_1,\ldots,z_n$.

In the These combined terms are each 
