...
\ur{The reuse of residual is bothersome, we should use different terminology,
1-value and 2-value perhaps.}
...
called, respectively, the {\em precomputation} and {\em residual}.%
...
\end{lstlisting}
splits into a precomputation \verb|(1+2,3+4)| and a residual
\verb|fn (L1,L2) => L1 < L2-5|, with the property that the residual applied to
...
the pair of these components: \verb|(((),3*4),1+2)|. The splitting residual is
...
$c$ and residual $l.r$, while $\splittwosym$ sends a stage \bbtwo\ term $e$ to a
precomputation $p$ and a residual $l.r$. In both cases, the term $e$ is open on
variables in the context $\Gamma$. We write the rules using pattern notation
when appropriate.
%For concision, we represent residuals as a term open on a single variable,
%rather than as functions.

\ur{Reformat rules to make them fit into the paper.}

\ur{Explain the rules.  This is the most important part of the
  paper. Work needed here.}
Every $\splittwosym$ rule works by bundling the precomputations of the constituent parts, and
then unbundling them with a pattern.
...
and projects out the integer result in the residual.%
\footnote{The residual of an integer expression is usually trivial, 
...
residual, we case on that injection, and resume the stage \bbtwo\ portion of the
correct branch.
...
the residual which handles all of the stage \bbtwo\ content. Note that stage
