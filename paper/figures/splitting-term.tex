\begin{figure*}
\begin{mathpar}
\fbox{Stage \bbone} 
\and
%\infersplitone [unit]			{\spl {()}{\rmunit}{((),()),\_.()}}															{\cdot} \and
%\infersplitone [int]			{\spl {i}{\rm int}{(i,()),\_.()}}															{\cdot} \and
\infersplitone [hyp]			{\spl {x}{A}{(x,()),\_.x}}																	{\cdot} \and
\infersplitone	 [\to I]	{\spl {\lam{x}{A}{e}}{A \to B}{(\lambda x.c,()),\_.(\lambda (x,l).r)}}					{\sub [\Gamma,\col x A] {} B} 									\and
\infersplitone [\to E]	{\spl {e_1~e_2}{B}{\left(
									\talllet{(y_1,z_1)}{c_1}{
									\talllet{(y_2,z_2)}{c_2}{
									\talllet{(y_3,z_3)}{y_1~y_2}{(y_3,(z_1,z_2,z_3))}}}\right),
							\begin{aligned}
									(l_1,&l_2,l_3).\\[-\baselineskip] % terrible hax
									&(r_1~(r_2,l_3))\\
							\end{aligned}
							}}																									{\sub 1 {A \to B} & \sub 2 A}							\and
\infersplitone [\times I] 	{\spl {(e_1,e_2)}{A\times B}
									{\left(
										\talllet{(y_1,z_1)}{c_1}{
										\talllet{(y_2,z_2)}{c_2}{
										((y_1, y_2),(z_1, z_2))}}
									\right),
									(l_1,l_2).(r_1,r_2)
							}}																				{\sub 1 A & \sub 2 B} \and
\infersplitone [\times E_1]	{\spl {\pio~e}{A}{\talllet{(y,z)}{c}{(\pio~y,z)},l.\pio~r}}								{\sub {} {A\times B}} \and
%\infersplitone [\times E_2]	{\spl {\pit~e}{B}{\talllet{(y,z)}{c}{(\pit~y,z)},l.\pit~r}}								{\sub {} {A\times B}} \and
%\infersplitone [let]			{\spl {\letin{x}{e_1}{e_2}}{B}{
%								\begin{array}{l}
%									\left(\talllet{(x,z_1)}{c_1}{
%									\talllet{(y,z_2)}{c_2}{
%									(y, (z_1, z_2))}}\right),
%									(l_1,l_2).\letin{x}{r_1}{r_2}
%								\end{array}}}																					{\sub 1 A & \sub [\Gamma,\col{x}{A}] 2 B} 		\and
%\infersplitone [+ I_1]	{\spl {\inl~e}{A+B}{\talllet{(y,z)}{c}{(\inl~y,z)},l.r}}			{\sub {} A}												\and
%\infersplitone [+ I_2]	{\spl {\inr~e}{A+B}{\talllet{(y,z)}{c}{(\inr~y,z)},l.r}}			{\sub {} A}												\and
\infersplitone [+ E]		{\spl {\left(\tallcase{e_1}{x_2.e_2}{x_3.e_3}\right)}{C}
								{\begin{array}{l}\left(
								\talllet{(y_1,z_1)}{c_1}{
									\tallcase{y_1}
									{x_2.\letin{(y_2,z_2)}{c_2}{(y_2,(z_1,\inl(z_2)))}}
									{x_3.\letin{(y_3,z_3)}{c_3}{(y_3,(z_1,\inr(z_3)))}}
								}\right), \\
								(l_1,l_b).\left(\talllet{z}{r_1}{
									\tallcase{l_b}
									{l_2.\letin{x_2}{z}{r_2}}
									{l_3.\letin{x_3}{z}{r_3}}}\right)\end{array}}}				{\sub 1 {A+B} & \sub [\Gamma,\col{x_2} A] 2 C 	& \sub [\Gamma,\col{x_3} B] 3 C} 	

\infersplitone [+ E]		{\spl {\left(\tallcase{e_1}{x_2.e_2}{x_3.e_3}\right)}{C}
								{\begin{array}{l}\left(
								\talllet{(y_1,z_1)}{c_1}{
									\tallcase{y_1}
									{x_2.\letin{(y_2,z_2)}{c_2}{(y_2,(z_1,\inl(z_2)))}}
									{x_3.\letin{(y_3,z_3)}{c_3}{(y_3,(z_1,\inr(z_3)))}}
								}\right), \\
								(l_1,l_b).\left(
									\tallcase{(r_1;l_b)}
									{l_2.\letin{x_2}{()}{r_2}}
									{l_3.\letin{x_3}{()}{r_3}}\right)\end{array}}}				{\sub 1 {A+B} & \sub [\Gamma,\col{x_2} A] 2 C 	& \sub [\Gamma,\col{x_3} B] 3 C} 	
%
%\infersplitone [\mu I]		{\spl {\roll~e}{\mu\alpha.\tau}{\talllet{(y,z)}{c}{(\roll~y,z)},l.r}}						{\sub {} {[\mu\alpha.\tau/\alpha]\tau}}							\and
%\infersplitone [\mu E]		{\spl {\unroll~e}{[\mu\alpha.\tau/\alpha]\tau}{\talllet{(y,z)}{c}{(\unroll~y,z)},l.r}}	{\sub {} {\mu\alpha.\tau}}										\and
%
%\infersplitone [if]			{\spl {\left( \tallif {e_1}{e_2}{e_3} \right)}{A} 
%								{\begin{array}{l}
%									\left(\talllet{(y_1,z_1)}{c_1}{\tallif {y_1}
%										{\letin{(y_2,z_2)}{c_2}{(y_2, (z_1, \inl~z_2))}}
%										{\letin{(y_3,z_3)}{c_2}{(y_3, (z_1, \inl~z_3))}}}
%									\right), \\
%									(l_1,l_b).(r_1;\caseof{l_b}{l_2.r_2}{l_3.r_3})
%								\end{array}}}																					{\sub 1 \rmbool & \sub 2 A & \sub 3 A }		\and

%\infersplitone	{\spl {\liftint~e} ? {c,l.r}}											{\sub {} ?}					

\end{mathpar}
\hrule
\begin{mathpar}
\fbox{Stage \bbtwo} 
\and
%\infersplittwo [int]			{\spl {i}{\rm int}{(),\_.i}}																	{\cdot} \and
%\infersplittwo [unit]			{\spl {()}{\rmunit}{(),\_.()}}																{\cdot} \and
\infersplittwo [hyp]			{\spl {x}{A}{(),\_.x}}																		{\cdot} \and
\infersplittwo [\times E_1]	{\spl {\pio~e}{A}{p,l.\pio~r}}																{\sub {} {A\times B}} \and
\infersplittwo [\to I]		{\spl {\lam{x}{A}{e}}{A \to B}{p,l.(\lam{x}{A}{r})}}										{\sub [\Gamma,\col x A] {} B} 									\and
\infersplittwo [\to E]		{\spl {e_1~e_2}{B}{(p_1,p_2),(l_1,l_2).r_1~r_2}}											{\sub 1 {A \to B} & \sub 2 A}							\and
\infersplittwo [\times I]	{\spl {(e_1,e_2)}{A\times B}{(p_1,p_2),(l_1,l_2).(r_1,r_2)}}								{\sub 1 A & \sub 2 B} \and
%\infersplittwo [\times E_2]	{\spl {\pit~e}{B}{p,l.\pit~r}}																{\sub {} {A\times B}} \and
%\infersplittwo [+ I_1]		{\spl {\inl~e}{A+B}{p,l.\inl~r}}										{\sub {} A}												\and
%\infersplittwo [+ I_1]		{\spl {\inr~e}{A+B}{p,l.\inr~r}}										{\sub {} A}												\and
%\infersplittwo [\mu I]		{\spl {\roll~e}{\mu\alpha.\tau}{p,l.\roll~r}}												{\sub {} {[\mu\alpha.\tau/\alpha]\tau}}							\and
%\infersplittwo [\mu E]		{\spl {\unroll~e}{[\mu\alpha.\tau/\alpha]\tau}{p,l.\unroll~r}}								{\sub {} {\mu\alpha.\tau}}										\and
\infersplittwo [+ E]			{\spl {\left(\caseof{e_1}{x_2.e_2}{x_3.e_3}\right)}{C}
								{(p_1,p_2,p_3), (l_1,l_2,l_3).\left(\caseof{r_1}
									{x_2.r_2}{x_3.r_3}\right)}}				{\sub 1 {A+B} & \sub [\Gamma,\col{x_2} A] 2 C 
																									& \sub [\Gamma,\col{x_3} B] 3 C} 								
%\infersplittwo [let]			{\spl {\letin{x}{e_1}{e_2}}{B}{(p_1,p_2), (l_1,l_2).\letin{x}{r_1}{r_2}}}				{\sub 1 A & \sub [\Gamma,\col{x}{A}] 2 B} 		\and
%\infersplittwo [if]			{\spl {\left(\tallif {e_1}{e_2}{e_3}\right)}{A} 
%								{(p_1,p_2,p_3),(l_1,l_2,l_3).\left(\tallif{r_1}{r_2}{r_2}\right)}}						{\sub 1 \rmbool & \sub 2 A & \sub 3 A }		\and
\end{mathpar}
\hrule
\begin{mathpar}
\fbox{Staging features}
\and
\infersplittwo [\fut E]		{\spl {\prev~e}{A} {\pit~c,l.r}}															{\splitonesub {} {\fut A}} 					\and
\infersplitone [\fut I]		{\spl {\next~e}{\fut A}{((),p),l.r}}														{\splittwosub {} A} 							\and
\infersplittwo [hold]			{\spl {\pause~e}{\rmint}{c,(a,l).(r;a)}}													{\splitonesub {} \rmint}						\and
\infersplitone	{\spl {\monoTerm~e} {\monoType A} {(e,()),\_.()}}						{\cdot}						\and
\infersplitone	{\spl {\letmono x{e_1}{e_2}} {?} 
				{\left(\talllet {(x,z_1)} {c_1} {
				 \talllet {(y_2,z_2)} {c_2} {(y_2,(z_1,z_2))}}\right),
					(l_1,l_2).(r_1;r_2)}}												{\sub 1 ? & \sub 2 ?}		\and
\infersplitone	{\spl {\lifttag~e} ? {c,l.r}}											{\sub {} ?}					\and
\end{mathpar}
\caption{Selected splitting rules.  At both stages, rules for \texttt{pi2}/\texttt{inl}/\texttt{inr}/\texttt{roll}/\texttt{unroll} follow same pattern as \texttt{pi1}.}
\label{fig:termSplit}
\end{figure*}

