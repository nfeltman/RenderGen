...
\item \emph{Statics}: M12 is a language with three worlds: 1M, 1P, and 2.
We model the computations described above as functions in world M, and their
results as values in 2.
...
\crem{Now that hold is not officially in the language, I think we have to put a
function prototype in the quickselect code figure. (And move the footnote about
it to the end of 3.1.}
...
Ignoring for the moment all \texttt{pure} annotations, the first-stage code in
...
\texttt{pure} annotations further distinguish those first-stage computations
which do not contain any embedded second-stage code. (Code \emph{inside}
\texttt{pure}, like the \texttt{part}ition function, is guaranteed not to
