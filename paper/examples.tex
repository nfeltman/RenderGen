...
Programs featuring staged structure are particularly important in real-time
computer graphics since modern graphics architectures \emph{require} that
graphics computations be organized into distinct {\em passes} which perform
increasingly fine-grained computations (e.g., per object, per screen region,
per pixel) where many computations in later passes use the
results from a single computation in an earlier pass\,\cite{OpenGL4Spec}.

While performant, programming with explicit passes results in complex
code where invariants must hold across different passes, and local
changes for one pass may require changes to other passes to ``plumb''
its inputs through all upstream passes. Thus, requiring manual
separation of passes breaks key programming abstractions such as
composition and modularity.  Graphics researchers therefore have
suggested using explicitly staged programs
...
each GPU pipeline stage. Composition of such functions in
...
A simple example of composition of staged functions (representing GPU
...
\end{lstlisting}
%
Whereas prior systems provides combinators like \texttt{par} as
