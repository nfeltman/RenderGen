...
\item Computing the shading of a surfacing in a 3D rendering, where some contributors to the final pixel color can be approximated as constant over $4\times4$ pixel blocks.  
\item Running a language interpreter on a single program for multiple inputs.
\end{itemize}

The goal is to run programs in a way that doesn't repeat work unless it depends on the varying input.  There are two separate problems at play here.  The first is {\em binding-time analysis}, the problem of determining {\em what} portions of the program to run statically versus dynamically.  After that is {\em staged execution}, the problem of {\em how} to execute in a staged way. We  consider only the latter in this paper.  That is, we assume that programs are given in a representation, covered later, that fully determines staged execution.

We consider two possible models of staged execution, 

\TODO partial eval

\TODO stage-splitting
