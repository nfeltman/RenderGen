...
Every expression in \lang\ has both a type and a {\em stage}, either \bbone~or \bbtwo.
Just as a type describes \emph{what} sort of value an expression reduces to, the stage tells us \emph{when} an expression will be fully reduced.  
Within each stage, \lang\ reduces to the simply-typed lambda calculus augmented with products, sums, general recursion, and base types
\footnote{It needn't be the case that we choose the same set of features at both levels, but this does simplify the exposition.}.

In addition to these types, stage-\bbone\ expressions may also be of type $\fut A$,
representing an enclosed stage-\bbtwo\ expression of type $A$. $\fut A$ terms must be treated opaquely by stage-\bbone\ code
as they cannot be reduced (to values with type $A$) until stage \bbtwo.
While $\fut$ allows use to embed stage-\bbtwo\ types within stage-\bbtwo\ types, 
there is no way to embed a stage-\bbone\ type within a stage-\bbtwo\ type.

Despite this one-way dependence between the two stages at the type level, 
stage-\bbone\ and stage-\bbtwo\ terms are defined mutually recursively:
...
%$e$), as well as two administrative contexts ($\Gamma$), covered
