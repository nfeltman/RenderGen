...
The input to our splitting algorithm is given in \lang, a typed, modal
...
considered in the partial evaluation and metaprogramming literature,
though it differs in three key ways:

\begin{enumerate}

\item We restrict our attention to a two-stage language to simplify our
presentation of the splitting algorithm, though we expect that the techniques
presented here extend to more stages.

\item Although our language describes computations with two {\em stages}
(\bbone\ and \bbtwo), its type system makes use of three {\em worlds}
(\bbonem, \bbonep, \bbtwo).  For reasons discussed later, we require
that stage \bbone\ computations be classified as being either pure
(occuring at \bbonep) or mixed (occuring at \bbonem). 
Only mixed code may transition to stage/world \bbtwo.
...
%Apart from the non-duplication, the dynamic semantics of \lang\ is essentially
%identical to Davies'.
...
To ensure that the staging features are only applied in a valid way, 
we index our judgments not only by a type, but also a stage $w$.
