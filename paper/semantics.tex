...
We now describe \lang, a simple two-stage language
for which we will later define and analyze a stage splitter.

For this purpose, we use {\em fast exponent} as a running example.
It calculates $b^p$ in $O(\log p)$ time.  
Using a standard unstaged ML-like syntax, it is
\begin{lstlisting} 
fun fexp (b : int, p : int) : int = 
	if p == 0 then
		1
...
such that information can flow from the stage \bbone\ portions of the term to the stage \bbtwo\ portions,
but never from \bbtwo\ to \bbone.
Our starting point for this is a typed lambda calculus with products, sums, and isorecursive types.
Here is the fast exponent example in staged form:
\begin{lstlisting} 
...
With this in mind, observe that the exponent decomposition and recursive calls in ${\tt fexp}$ 
all occur in stage \bbone, whereas squaring and multiplication by the base occur at stage \bbtwo.

This paper investigates a transformation called stage-splitting, 
wherein a single multi-stage program is split into multiple single-stage programs.
For ${\tt fexp}$, this is:
