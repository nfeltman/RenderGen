...
Therefore, a suitably staged dynamic semantics for \lang\ must evaluate
...
To gain intuition about the challenges of ensuring staged evaluation of \lang, consider the following
example:
\begin{lstlisting}
#2 (next {1+2}, 3+4)
\end{lstlisting}
...
To fully evaluate an expression in \lang, we first evaluate all of its stage
\bbone\ subexpressions as described above. This results in a stage \bbtwo\
expression with no stage \bbone\ content; we say that it is a term in a
monostaged%
...
language called \langTwo. Ordinary \langTwo\ evaluation, implemented by the
$\tworedsym$ judgment, completes the evaluation of this term.
