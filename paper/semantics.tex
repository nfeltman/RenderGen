...
\ur{Partial values and residuals should be ``definitons'', i.e.,
  use italics here.}
...
\end{lstlisting}
is a residual,
\ur{Comma above! ``classification'' sounds odd to me.  just say they
  ``are stage-1 terms.''}
...
\ur{I would recommend using ``1-value'' and ``2-value'' instead of
  partial values and residuals.  The former are more accurate, more
  direct, more succinct, and does not lead to confusions with other
  residuals.}

\ur{Many incorrectly hyphenated ``partial values'' in this text.}
...
\ur{This is really a continuation of the previous paragraph.}
...
\ur{This might be standard in the PE literature.  If that is the case
  there is no need to elaborate as much.}
...
\ur{The following several paragraphs should probably be somewhere
  else, further ahead, or further back when we discuss the
