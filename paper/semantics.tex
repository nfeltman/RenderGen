...
Without this, there would be no way to get information out from under a $\curr$ wrapper.
...
Since a residual $q$ is monostage by definition, this second pass is just standard unstaged evaluation.
Consequently, this paper does not describe in any detail the second pass of evaluation.
...
Since first pass evaluation should not reduce stage two terms, this judgement does nothing but search for \prev\ blocks within $e$.
...
One consequence of the stronger requirement on partial values is that we will need a new kind of step to put terms into that form.
