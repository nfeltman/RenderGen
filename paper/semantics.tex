...
% We chose to keep these as two separate contexts, because only $\Xi$ is needed for well-formedness of partial values.
...
$\xi'\wf$,
$\Xi,\dom{\xi'};\Gamma\vdash \coltwo{e}{B}$, and
$\Xi,\dom{\xi'};\Gamma\vdash e \res$.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
\item $\xi\wf$;
\item $\Xi,\dom\xi;\cdot\vdash \colone{v}{A}$; and
\item $\Xi,\dom\xi;\cdot\vdash v\pval$.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
\item $\Xi;\Gamma\vdash \coltwo{q}{A}$; and
\item $\Xi;\Gamma\vdash q\val$.
\end{enumerate}
\end{theorem}
