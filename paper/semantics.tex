...
One option, taken by \cite{davies96}, is to treat $\next\{\mathtt{1+2}\}$ as a value for the purposes of stage-\bbone~evaluation, 
...
$\next$ block bound to it. Our solution is to replace the $\next$ with a new variable and create an
explicit substitution (shown with a $\mapsto$) binding that variable to the $\next$'s contents.  
...
%		}	
%\end{lstlisting}
\TODO code written, but listings don't work in footnotes! commented out in source
}
