...
terms are evaluated; in the second stage all stage-2 terms are
evaluated.  To eliminate duplication, our semantics lifts out each
stage-2 term nested inside of a stage-1 expressions, into table which
makes it possible for the stage-2 term, actually the corresponding
residual obtained by reducing all stage-1 subterms, to be referred to
by use a dynamically generated variable (a label).

Since stage-1 evaluation does not evaluate stage-2 terms and since
stage-2 terms may be nested inside stage-1 terms, stage-1 evaluation
cannot reduce \lang\ terms to values but only to {\em partial values},
which are stage~\bbone\ terms that have been evaluated except for
their stage-2 subterms, which can only be~\bbtwo\ variables wrapped in
$\next$ blocks.  \figref{validPvalues} defines partial values.


The dynamic semantics consist of two evaluation relations $\redonesym$
and $\redtwosym$, called {\em 1-evaluation} and {\em 2-evaluation},
that respectively evaluates stage-1 and stage-2 expressions. 
...
The relation 1-evaluation maps a stage-1 term to a partial value and a
residual table. 
