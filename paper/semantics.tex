...
namely either stage-\bbone~a stage-\bbtwo, in addition to a type.
Just as the type tells us {\em what} an expression will reduce to,
a stage tells us {\em when} it will fully reduce. 
The reason for a two-stage restriction (as opposed to \cite{davies96}, which allows an infinite number of stages) will be discussed later.
...
and our concrete syntax, we surround their arguments with \{braces\} to clearly indicate the stage boundary.
...
The $\next$ expression allows us to embed [kayvon wants a different word here] stage-\bbtwo~expression in stage-\bbone~expression, 
...
Below is an example of fast-exponent written in \lang.  
The function excepts an exponent integer at the first stage, 
a base ($b$) has type $\fut int$, indicated that it's content is accessible only at the second stage,
and produces a circle int at stage two.
The code works by squaring if the exponent is even, and multiplication by the base if the exponent is odd.
The type system here is sufficient to prove that all of the decomposition of the exponent can be done at the first stage.
...
...segue into semantics here... talk about residual generation...
