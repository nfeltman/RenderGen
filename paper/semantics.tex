...
%
\texttt{qss} has a more precise type---it is a first-stage function which
...
\texttt{partition} it. The \texttt{case} term in \texttt{qSelect} depends
on \texttt{k}, whose type $\fut\rmint$ indicates that it is only available to
second-stage computations. Since \texttt{qss} itself produces a
$\fut\rmint$, the remainder of the function is second-stage code.

The $\next$ wraps a second-stage term of type $\rmint$ (the \texttt{case}
term) as a first-stage term of type $\fut\rmint$ (the result 
of \texttt{qss}). Inside the \texttt{LT} and \texttt{GT} branches, the
$\prev$ unwraps first-stage $\fut\rmint$s (the results of the recursive calls
to \texttt{qss}) as second-stage $\rmint$s. In the \texttt{EQ} branch,
