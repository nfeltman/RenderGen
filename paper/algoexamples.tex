...
1`//dot : vec * vec -> $int
fun dot (
  (3`grnd{`1`xliteralone`3`}`1`,`3`grnd{`1`yliteralone`3`}`1`,next{`2`zliteralone`1`}): vec,
  (3`grnd{`1`xliteraltwo`3`}`1`,`3`grnd{`1`yliteraltwo`3`}`1`,next{`2`zliteraltwo`1`}): vec) = 
next{
...
\crem{You could map qs and it would work.}
...
used to generate monostage code required for each GPU pipeline stage
...
% in language support for composing pipelines

% like functionality achieved Spark via mixin

% there is actually a use for this.
...
1`fun shade (refl : object * $2`coord` -> $2`color`,
      albedo : object * $2`coord` -> $2`color`)
        (obj : object, next{2`xy`} : $2`coord`) : $2`color` =
