...
\subsection {Combining Pixel Computations}

\crem{Text is out of sync with example.}
...
1`fun sum (f : object * $2`coord` -> $2`color`,
         g : object * $2`coord` -> $2`color`)
        (x : object, next{2`c`} : $2`coord`) : $2`color` =
  2`prev{`1`f (x,next{2`c`})`2`} + prev{`1`g (x,next{2`c`})`2`}`
\end{lstlisting}
