...
How does our algorithm emit a binary search tree and traversal algorithm? The
$\splittwosym$ rule for \texttt{case} tuples up the precomputations for its
branches, and in the residual, selects the residual corresponding to the
appropriate branch.

All of the list partitioning work now occurs within the precomputation.

Thus, {\tt qs1} performs $\Theta(n \log n)$ work per call, whereas {\tt qs2}
performs expected $\Theta(\log n)$ work.  This results in a net speedup over
standard quickselect if we perform $\Omega(n / \log n)$ queries on the same
list. 

This is like the previous example, except that this is actually useful, and
creates a \emph{different}, interesting, known algorithm. 
As before, the tree matches the structure of the code. We could partially
evaluate also, but if the list is big, that's a bad idea.

In this example, we
make use of the behavior where first stage code is executed even under second
stage branches.

\crem{Nico: add something here about the importance of ground}

The helper function {\tt partition} operates entirely in
stage \bbone, and so we have wrapped it in a \texttt{grnd}.  
