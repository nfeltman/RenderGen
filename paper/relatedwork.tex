...
In the partial evaluation community, metastatic staging transforms (called \emph{program bifurcation} and \emph{binding time separation}) 
similar to splitting have been investigated 
as a preprocessing step necessary for simplifying the specialization phase of self-applied partial evaluators. 
For instance, Mogensen\,\cite{Mogensen89a} and Niel et al.\,\cite{DeNiel91} defined transformations for a set of first-order recursive functions.

Those works operate on the results of binding time analysis


and so much of their binding-time specification just amounts to a type system.
Formulations like \lang\ (namely the two-level system of~\cite{NN92-twolevel}) were known at the time,
but~\cite{Mogensen89a} found these unsatisfactory because they could not describe data structures with mixed stage.

Davies essentially solved this limitation in~\cite{davies96} 
...
even simple features such as functions.
