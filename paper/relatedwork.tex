...
(compile-time) and dynamic (run-time) stages. Given a program and values 
for all static inputs (inputs available at compilation
...
and writing ``meta programs'' that, when evaluated at a stage, yield
...
system via the Curry-Howard isomorphism applied to temporal logic.
...
by developing statically typed programming language based on ML that
...
Another class of pre-computation techniques is incremental
...
change. Thus, its benefits are dependent the input changes made and
...
However, they only consider a simple first-order language and
...
which would prevent optimization of programs such as quickselect.
...
components by frequency of execution (Separation into a pre-determined
...
limited to simple, imperative languages. We are not aware of prior
work the presents a splitting algorithm in the context of higher-order
