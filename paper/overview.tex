...
a sorted list \texttt{s} and then perform lookups in \texttt{s}.
Unfortunately, lookups in \texttt{s} are also $\Theta(n)$ on average, since
...
subtree. For a sufficiently balanced tree, 
which one would get with a random choice of pivot, this approach has 
